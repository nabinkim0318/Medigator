...
def start(body: StartIn):
    ...
    with sqlite3.connect("data/app.db") as c:
        c.execute("""INSERT INTO intake_session(id, token, status, patient_hint, expires_at)
                     VALUES(?,?,?,?,?)""", (sid, token, "PENDING", body.patient_hint, expires_at))
        c.commit()
...
def load(token: str):
    v = verify(token)
    if not v: raise HTTPException(401, "invalid token")
    with sqlite3.connect("data/app.db") as c:
        row = c.execute("SELECT id,status,expires_at FROM intake_session WHERE token=?", (token,)).fetchone()
    if not row: raise HTTPException(404, "not found")
    sid, status, exp = row
    if status != "PENDING": raise HTTPException(410, "already used")
    if datetime.fromisoformat(exp) < datetime.utcnow().replace(tzinfo=None):
        raise HTTPException(410, "expired")
    # Fixed 6-question example
    questions = [{"id":"cc","label":"What is your main symptom?"},
                 ...
                 {"id":"associated","type":"multi","options":["shortness of breath","diaphoresis","nausea"]},
...
def submit(token: str, body: SubmitIn):
    ...
    with sqlite3.connect("data/app.db") as c:
        ...
        if datetime.fromisoformat(exp) < datetime.utcnow().replace(tzinfo=None):
            raise HTTPException(410, "expired")
        c.execute("INSERT OR REPLACE INTO intake_payload(session_id, answers_json) VALUES(?, json(?))",
                  (sid, __import__("json").dumps(body.answers)))
        c.execute("UPDATE intake_session SET status='SUBMITTED', submitted_at=datetime('now') WHERE id=?", (sid,))
        c.commit()
