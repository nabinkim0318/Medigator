...
from api.services.rag import retrieve, init_retriever, make_query, build_index, RAGStore
...
async def search_documents(request: SearchRequest):
    ...
    try:
        ...
        # Perform search
        results = retrieve(summary, k=request.k)
...
async def build_rag_index(request: IndexRequest):
    ...
    logger.info(f"RAG index build requested: docs_dir={request.docs_dir}, out_dir={request.out_dir}")
...
async def get_query_example():
    ...
    try:
        ...
        logger.error(f"Query example generation failed: {str(e)}")
        raise RAGServiceException(
