...
from openai import OpenAI, AsyncOpenAI, APIConnectionError, RateLimitError, InternalServerError
...
LLM_TEMPERATURE = float(getattr(settings, "LLM_TEMPERATURE", getattr(settings, "openai_temperature", 0.1)))
LLM_TIMEOUT_S = float(getattr(settings, "LLM_TIMEOUT_MS", getattr(settings, "openai_timeout_ms", 3500)))/1000.0
...
def _ckey(messages: list[dict], model: str, temperature: float, response_format: Optional[dict]) -> str:
    blob = json.dumps({"m": messages, "model": model, "t": temperature, "rf": response_format}, sort_keys=True)
    return hashlib.sha256(blob.encode()).hexdigest()

# ---- Enhanced Retry helpers ----
def _is_retryable(e: Exception) -> bool:
    """Check if exception is retryable with more specific error handling"""
    return isinstance(e, (APIConnectionError, RateLimitError, InternalServerError, TimeoutError, asyncio.TimeoutError)) \
        or any(k in str(e).lower() for k in ["rate limit","timeout","connection","server error","unavailable","temporary","retry"])
...
async def chat_json(
    *,
    ...
    use_cache: bool = True,
    fallback_func: Optional[Callable[[], Dict[str, Any]]] = None,
) -> dict:
    """
...
def get_client_status() -> dict:
    ...
    return {
        ...
        "cache": cache_stats,
        "clients": {
            "sync": sync_client is not None,
            "async": async_client is not None
        }
    }
