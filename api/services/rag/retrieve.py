...
_tokenized: Optional[List[List[str]]] = None
...
def make_query(summary: Dict[str, Any]) -> str:
    ...
    if flags.get("ischemic_features"):
        parts += ["chest pain", "ischemia", "ECG", "troponin", "risk stratification", "outpatient evaluation"]
    if flags.get("dm_followup"):
        parts += ["type 2 diabetes", "HbA1c frequency", "lipid management", "ADA standards"]
...
def _merge_scores(
    ...
    merged = []
    for i, e, b in zip(idxs, emb_norm, bm_norm):
...
def retrieve(summary: Dict[str, Any], k: int = RAG_TOPK) -> List[Retrieval]:
    ...
    try:
        ...
        emb_hits: List[Tuple[int, float]] = _store.search(q_emb, top_k=max(8, k * 2))  # [(idx, score)]
        ...
        if _bm25 is not None and _tokenized is not None:
            ...
            bm_hits = sorted(list(enumerate(scores)), key=lambda x: x[1], reverse=True)[:max(8, k * 2)]
        ...
        for idx, score in top:
            meta = _store.get_meta(idx)  # {'id','title','source','text','file','start','end','url',...}
            # Retrieval type: {"chunk": meta, "score": float}
            results.append({"chunk": meta, "score": float(round(score, 4))})
        return results
