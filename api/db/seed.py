...
def create_sample_data(conn):
    ...
    em_rules_data = [
        ('High risk patient identification', 'Rule to identify high risk patients', 'age > 65 AND chronic_conditions > 2', 'flag_high_risk', 1),
        ('Emergency room priority', 'Rule to determine emergency room priority', 'vital_signs_abnormal = true', 'assign_priority_1', 2),
        ('Provider notification', 'Rule to notify provider', 'critical_lab_result = true', 'notify_provider', 3)
    ]
...
def main():
    ...
    try:
        ...
        # JSON data load
        json_files = [
            (fhir_dir / "cp_bundle.json", "patients")
        ]
