...
def log_security_event(event_type: str, details: str, user_id: str = None):
    ...
    security_logger.warning(f"Security event: {event_type} - {details} - User: {user_id or 'Unknown'}")


def log_performance_metric(operation: str, duration: float, details: str = None):
    """Log performance metrics"""
    logger = logging.getLogger("api.performance")
    logger.info(f"Performance: {operation} took {duration:.3f}s - {details or ''}")


def log_phi_access(operation: str, patient_id: str = None):
